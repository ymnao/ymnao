...
loop do
  begin
    while (key = $stdin.read_nonblock(1))
  ...
  tick += 1
  next_spawn -= 1
  if next_spawn <= 0 && (cacti.empty? || cacti.last[0] < WIDTH - 40)
    cacti << [WIDTH - 5, rand < 0.65 ? :small : :big]
    next_spawn = 55 + rand(75)   # steady but not metronomic
  end
  cacti.each { |c| c[0] -= SPEED }
  cacti.reject! { |x, _| x < -5 }

  # gem hitbox: 7 columns around center, bottom row at height y
  hit = cacti.any? do |x, kind|
    x <= PLAYER_X + 3 && x + 4 >= PLAYER_X - 3 && y.round < CACTUS[kind].length
  end
  if hit
  ...
  # cacti
  cacti.each { |x, kind| draw(grid, x, CACTUS[kind]) }
